...
void GenAlgForm::MyForm::eliteSelection(bool** a, int *popNum, int *maxCros, int* exTrueCros) {
    ...
    *out += "Элитная селекция. Средняя приспособленность текущей популяции: " + std::to_string(*avrFit) + "\r\n";
    //определяем число "лучших" хромосом, приспособленность которых будет выше средней.
...
{  
    ...
    *out += "Начальная популяция (в скобках указаны фенотип и значение целевой функции):\r\n\r\n";
    ...
    //вывод ответа:
    if (numFit) *ans = " Оптимальное решение: " + std::to_string(ft(pop[0])) + "\r\n Значение функции: "+ std::to_string(tf(ft(pop[0])));
